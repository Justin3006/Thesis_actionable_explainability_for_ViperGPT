...
class GLIPDemo(object):
    ...
    def build_tensor_transforms(self):
        """
        Creates a basic transformation that was used to train the models past being made tensors
        after they are loaded as [C, H, W]
        """
        cfg = self.cfg

        # we are NOT loading images with OpenCV, so we do need to convert them
        # to BGR! 
        # if cfg.INPUT.TO_BGR255:
        #     to_bgr_transform = T.Lambda(lambda x: x * 255)
        # else:
        ...
        reorder_nchw = T.Lambda(lambda x: x.permute(2, 0, 1))
